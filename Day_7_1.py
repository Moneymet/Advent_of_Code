...
def recursive_bag_count(bag_node):
    #If bag has parents
    if len(bag_node.parent_nodes) != 0:
        for x in bag_node.parent_nodes:
            #Counts each parent and starts counting parents' parents
            counted_tree_dict[x.name] = x
            recursive_bag_count(x)

def bag_count_containing_bag(bag_name):
    #Gets node for main bag
    root_bag = tree_dict[bag_name]
    #Starts recursive loop going through entire line of parents of main bag
    recursive_bag_count(root_bag)
    #Returns length of counted bag dictionary, ensures no duplications
    return len(counted_tree_dict)
